...
HRESULT InitDevice()
{
    ...
    hr = CompileShaderFromFile(L"GigaChadEngine.fx", "VS", "vs_4_0", &pVSBlob);
    if (FAILED(hr))
    {
        MessageBox(NULL,
    ...
    hr = CompileShaderFromFile(L"GigaChadEngine.fx", "PS", "ps_4_0", &pPSBlob);
    if (FAILED(hr))
    {
        MessageBox(NULL,
            L"El archivo FX no se pudo compilar. Ejecuta el ejecutable desde el directorio que contiene el archivo FX.", L"Error", MB_OK);
    ...
    hr = CompileShaderFromFile(L"GigaChadEngine.fx", "ShadowPS", "ps_4_0", &pShadowPSBlob);
    if (FAILED(hr))
    {
        MessageBox(NULL,
